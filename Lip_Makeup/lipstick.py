...
class LipMakeup_class():
    ...
    def prepare(self):
        ...
        figure()
        height, width = self.imOrg.shape[:2]
        print(self.imOrg.shape[:2])

        imRoi = self.im.copy()
        gray = cv2.cvtColor(self.imOrg, cv2.COLOR_BGR2GRAY)
        ...
        for i in range(int(i_u_r[1][0]), int(i_u_r[1][-1] + 1)):
            xt(o_u_r[0](i), i_u_r[0](i) + 1, i)
            ext(i_l[0](i), o_l[0](i) + 1, i)

        # Now x and y contains coordinates of all the points on lips

        self.val = color.rgb2lab((self.imOrg[x, y] / 255.).reshape(len(x), 1, 3)).reshape(len(x), 3)
    ...
    def apply_lipstick_func(self,imageFile,r,g,b):
        ...
        self.prepare
        print(self.imOrg.flags)
        #imOrg.setflags(write=True)
        imOrg[x, y] = color.lab2rgb(self.val.reshape(len(x), 1, 3)).reshape(len(x), 3) * 255
